...
import { ArrowLeft, Heart, Share2, ShoppingCart, RotateCcw, ZoomIn, Maximize2, Eye, Loader2 } from 'lucide-react';
                    ...
                    <div style={{ background: '#111', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, overflow: 'hidden' }}>
                        <div style={{ position: 'relative', overflow: 'hidden', cursor: zoomed ? 'zoom-out' : 'zoom-in' }} onClick={() => setZoomed(z => !z)}>
                            <img
                                src={art.img}
                                alt={art.title}
                                style={{ width: '100%', height: 440, objectFit: 'cover', display: 'block', transition: 'transform 0.5s', transform: zoomed ? 'scale(1.4)' : 'scale(1)' }}
                            />
                        ...
                        <div style={{ padding: '14px 16px', display: 'flex', justifyContent: 'center', gap: 10 }}>
                            {[
                                { icon: <RotateCcw size={15} />, label: 'Rotate' },
                                { icon: <ZoomIn size={15} />, label: 'Zoom', action: () => setZoomed(z => !z) },
                                { icon: <Maximize2 size={15} />, label: '3D View' },
                            ].map((ctrl, i) => (
                ...
                <div>
                    ...
                    <div style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${art.accent}44`, borderRadius: 18, padding: '24px', marginBottom: 24, position: 'relative', overflow: 'hidden' }}>
                        ...
                        <div style={{ display: 'flex', gap: 12 }}>
                            <button onClick={() => setInCart(c => !c)} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: inCart ? '#15803d' : `linear-gradient(135deg, ${art.accent}, #a855f7)`, border: 'none', color: '#fff', fontWeight: 700, fontSize: 15, padding: '14px', borderRadius: 12, cursor: 'pointer', boxShadow: `0 0 20px ${art.accent}44`, transition: 'all 0.3s' }}>
                                <ShoppingCart size={18} />
                                {inCart ? '✓ Added to Cart' : 'Add to Cart'}
                            </button>
                            <button style={{ flex: 1, background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.15)', color: '#fff', fontWeight: 600, fontSize: 15, padding: '14px', borderRadius: 12, cursor: 'pointer', transition: 'background 0.2s' }}
